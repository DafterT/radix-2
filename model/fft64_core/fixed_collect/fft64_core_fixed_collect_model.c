...
#include "fft64_core_fixed_collect_butterfly.h"

#define FFT64_FIXED_BANK_COUNT 2U
#define FFT64_FIXED_BANK_DEPTH (FFT64_CORE_SIZE / FFT64_FIXED_BANK_COUNT)
...
typedef struct {
    fft64_core_cpx_q22_0_t banks[FFT64_FIXED_BANK_COUNT][FFT64_FIXED_BANK_DEPTH];
...
static fft64_core_cpx_q22_0_t fft64_fixed_memory_read(
...
static void fft64_fixed_bit_reverse_copy(
    const fft64_core_cpx_q16_0_t input[FFT64_CORE_SIZE],
    fft64_fixed_memory_t *memory
) {
...
static void fft64_fixed_execute_stage(
    ...
    for (int block_start = 0; block_start < FFT64_CORE_SIZE; block_start += stage_size) {
        for (int j = 0; j < half_stage; ++j) {
            ...
            fft64_core_cpx_q22_0_t top = fft64_fixed_memory_read(memory, (unsigned)top_index);
            fft64_core_cpx_q22_0_t bottom = fft64_fixed_memory_read(memory, (unsigned)bottom_index);

            fft64_core_fixed_collect_butterfly(
                &top,
                &bottom,
                twiddle_index
            );
...
static void fft64_fixed_collect_result(
    ...
    for (unsigned i = 0; i < FFT64_CORE_SIZE; ++i) {
        fft64_core_cpx_q22_0_t value = fft64_fixed_memory_read(memory, i);

        result->bins[i] = (fft64_core_cpx_q16_0_t){
...
fft64_core_fixed_result_t fft64_core_fixed_collect_eval(
    const fft64_core_cpx_q16_0_t input[FFT64_CORE_SIZE]
) {
