...
static void print_complex_vector(const char *title, const fft64_core_complex_t data[FFT64_CORE_SIZE]) {
    printf("%s\n", title);
...
int main(void) {
    ...
    fft64_core_result_t fft_result;

    srand(1);

    fill_noise_signal(input_signal);
    fft_result = fft64_core_reference_eval(input_signal);
