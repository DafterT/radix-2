...
#include "fft64_core_fixed_div2_butterfly.h"
...
typedef struct {
    fft64_core_cpx_q16_0_t banks[FFT64_FIXED_BANK_COUNT][FFT64_FIXED_BANK_DEPTH];
...
static fft64_core_cpx_q16_0_t fft64_fixed_memory_read(
...
static void fft64_fixed_execute_stage(
    ...
    for (int block_start = 0; block_start < FFT64_CORE_SIZE; block_start += stage_size) {
        for (int j = 0; j < half_stage; ++j) {
            ...
            fft64_core_cpx_q16_0_t top = fft64_fixed_memory_read(memory, (unsigned)top_index);
            fft64_core_cpx_q16_0_t bottom = fft64_fixed_memory_read(memory, (unsigned)bottom_index);

            fft64_core_fixed_div2_butterfly(
                &top,
                &bottom,
                twiddle_index,
                apply_div2
...
static void fft64_fixed_execute_all_stages(
    ...
    for (int stage_size = 2; stage_size <= FFT64_CORE_SIZE; stage_size *= 2) {
        fft64_fixed_execute_stage(memory, stage_size, (stage_index & 1) != 0);
        stage_index += 1;
    }
}
...
fft64_core_fixed_result_t fft64_core_fixed_div2_eval(
    const fft64_core_cpx_q16_0_t input[FFT64_CORE_SIZE]
) {
