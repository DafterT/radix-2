...
#include "../fixed_collect/fft64_core_fixed_collect_model.h"
#include "../fixed_div2/fft64_core_fixed_div2_model.h"
#include "../fixed_sqrt2/fft64_core_fixed_sqrt2_model.h"
...
static fft64_core_cpx_q16_0_t compare_make_fixed_input(double input_scale) {
    fft64_core_cpx_q16_0_t sample;
...
static fft64_core_complex_t compare_fixed_to_double(fft64_core_cpx_q16_0_t value) {
    fft64_core_complex_t result;
...
static double compare_reference_output_scale(void) {
    ...
    for (int stage_size = 2; stage_size <= FFT64_CORE_SIZE; stage_size *= 2) {
        scale *= 1.0 / sqrt(2.0);
    }
...
static double compare_cpx_error_mag(fft64_core_complex_t reference, fft64_core_complex_t actual) {
    return hypot(reference.re - actual.re, reference.im - actual.im);
}

static double compare_cpx_power(fft64_core_complex_t value) {
...
int main(void) {
    ...
    printf("FFT64 core size    = %d\n", FFT64_CORE_SIZE);
    printf("Runs               = %d\n", FFT64_COMPARE_RUNS);
    printf("Bins per run       = %d\n", FFT64_CORE_SIZE);
    printf("Total bins         = %d\n", FFT64_COMPARE_RUNS * FFT64_CORE_SIZE);
    ...
    for (int run = 0; run < FFT64_COMPARE_RUNS; ++run) {
        fft64_core_cpx_q16_0_t fixed_input[FFT64_CORE_SIZE];
        fft64_core_complex_t reference_input[FFT64_CORE_SIZE];
        fft64_core_fixed_result_t fixed_sqrt2_output;
        fft64_core_fixed_result_t fixed_div2_output;
        fft64_core_fixed_result_t fixed_collect_output;
        fft64_core_result_t reference_output;
        ...
        fixed_sqrt2_output = fft64_core_fixed_sqrt2_eval(fixed_input);
        fixed_div2_output = fft64_core_fixed_div2_eval(fixed_input);
        fixed_collect_output = fft64_core_fixed_collect_eval(fixed_input);
        reference_output = fft64_core_reference_eval(reference_input);

        for (int bin = 0; bin < FFT64_CORE_SIZE; ++bin) {
            fft64_core_complex_t fixed_sqrt2_bin = compare_fixed_to_double(fixed_sqrt2_output.bins[bin]);
            fft64_core_complex_t fixed_div2_bin = compare_fixed_to_double(fixed_div2_output.bins[bin]);
            fft64_core_complex_t fixed_collect_bin = compare_fixed_to_double(fixed_collect_output.bins[bin]);
            fft64_core_complex_t reference_sqrt2_bin = {
                .re = reference_output.bins[bin].re * reference_output_scale_sqrt2,
                .im = reference_output.bins[bin].im * reference_output_scale_sqrt2
            };
            fft64_core_complex_t reference_div2_bin = {
                .re = reference_output.bins[bin].re * reference_output_scale_div2,
                .im = reference_output.bins[bin].im * reference_output_scale_div2
            };
            fft64_core_complex_t reference_collect_bin = {
        ...
        if (FFT64_COMPARE_PRINT_RUNS != 0) {
            printf(
                ...
                run,
                sqrt2_run_error_mag / (double)FFT64_CORE_SIZE,
                sqrt2_run_max_error_mag,
                sqrt2_signal_power,
                sqrt2_noise_power,
                sqrt2_run_sqnr_db,
                div2_run_error_mag / (double)FFT64_CORE_SIZE,
                div2_run_max_error_mag,
                div2_signal_power,
                div2_noise_power,
                div2_run_sqnr_db,
                collect_run_error_mag / (double)FFT64_CORE_SIZE,
