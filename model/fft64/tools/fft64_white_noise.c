...
static double fft64_white_noise_random_unit_sample(void) {
    return -1.0 + (2.0 * (double)rand() / (double)RAND_MAX);
}

static int16_t fft64_white_noise_round_clip_q16_0(double value_q16_0) {
...
double fft64_white_noise_input_scale(double backoff_db) {
...
void fft64_white_noise_generate_frame(
    fft64_fixed_cpx_q16_0_t output[FFT64_FIXED_SIZE],
    uint32_t base_seed,
    uint32_t frame_index,
    double backoff_db
) {
    const double input_scale = fft64_white_noise_input_scale(backoff_db);

    srand((unsigned)(base_seed + frame_index));

    for (int i = 0; i < FFT64_FIXED_SIZE; ++i) {
        output[i].re = fft64_white_noise_round_clip_q16_0(
            fft64_white_noise_random_unit_sample() * input_scale
        );
        output[i].im = fft64_white_noise_round_clip_q16_0(
            fft64_white_noise_random_unit_sample() * input_scale
        );
    }
}
