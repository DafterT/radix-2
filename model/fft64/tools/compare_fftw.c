...
static void copy_to_fftw_input(
    const fft64_complex_t input_signal[FFT64_SIZE],
    fftw_complex fftw_input[FFT64_SIZE]
) {
    for (int i = 0; i < FFT64_SIZE; ++i) {
...
int main(void) {
    ...
    fft64_result_t my_result;
    fftw_complex fftw_input[FFT64_SIZE];
    fftw_complex fftw_output[FFT64_SIZE];
    ...
    my_result = fft64_radix2(input_signal);
    ...
    printf("avg |delta| = %.12e\n", sum_delta_mag / (double)FFT64_SIZE);
