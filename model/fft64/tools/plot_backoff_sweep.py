...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sweep WHITE_NOISE_BACKOFF_DB and plot fixed FFT64 comparison metrics."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="FFT64_COMPARE_RUNS value passed into compare_fixed.c.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def build_compare_command(root: Path, runs: int, backoff_db: int) -> list[str]:
    ...
    if os.name == "nt":
        ...
        bash_command = (
            ...
            f"make -s -C model CFLAGS={shlex.quote(cflags)} fft_fixed_compare"
        )
...
def main() -> int:
    ...
    write_csv(rows, output_dir / "fft64_backoff_sweep.csv")
    plot_rows(
        rows,
        output_dir / "fft64_backoff_sweep_full.png",
        f"FFT64 Fixed Compare Sweep: {args.start}..{args.stop} dB",
    )
    ...
    if zoom_rows:
        plot_rows(
            zoom_rows,
            output_dir / "fft64_backoff_sweep_zoom_8_20.png",
            f"FFT64 Fixed Compare Sweep: {args.zoom_start}..{args.zoom_stop} dB",
        )
