...
static radix2_butterfly_fixed_cpx_q16_0_t compare_make_fixed_input(double input_scale) {
    radix2_butterfly_fixed_cpx_q16_0_t sample;
...
static radix2_butterfly_reference_cpx_t compare_fixed_to_double(radix2_butterfly_fixed_cpx_q16_0_t value) {
    radix2_butterfly_reference_cpx_t result;
...
static double compare_cpx_power(radix2_butterfly_reference_cpx_t value) {
...
static void compare_print_fixed_cpx(const char *name, radix2_butterfly_fixed_cpx_q16_0_t value) {
    printf("  %-9s re=%10d im=%10d\n", name, value.re, value.im);
}

static void compare_print_double_cpx(const char *name, radix2_butterfly_reference_cpx_t value) {
...
int main(void) {
    ...
    for (int run = 0; run < STAGE_COMPARE_RUNS; ++run) {
        radix2_butterfly_fixed_model_t fixed_model = {0};
        radix2_butterfly_reference_model_t reference_model = {0};
        ...
        for (int call = 0; call < STAGE_CALLS_PER_RUN; ++call) {
            ...
            radix2_butterfly_fixed_cpx_q16_0_t a_fixed = compare_make_fixed_input(input_scale);
            radix2_butterfly_fixed_cpx_q16_0_t b_fixed = compare_make_fixed_input(input_scale);
            radix2_butterfly_reference_cpx_t a_reference = compare_fixed_to_double(a_fixed);
            radix2_butterfly_reference_cpx_t b_reference = compare_fixed_to_double(b_fixed);
            radix2_butterfly_fixed_output_t fixed_out = radix2_butterfly_fixed_step(&fixed_model, a_fixed, b_fixed, last_i);
            radix2_butterfly_reference_output_t reference_out = radix2_butterfly_reference_step(&reference_model, a_reference, b_reference, last_i);
            radix2_butterfly_reference_cpx_t fixed_a = compare_fixed_to_double(fixed_out.a);
            radix2_butterfly_reference_cpx_t fixed_b = compare_fixed_to_double(fixed_out.b);
            radix2_butterfly_reference_cpx_t err_a;
            radix2_butterfly_reference_cpx_t err_b;

            err_a.re = reference_out.a.re - fixed_a.re;
            err_a.im = reference_out.a.im - fixed_a.im;
            err_b.re = reference_out.b.re - fixed_b.re;
            err_b.im = reference_out.b.im - fixed_b.im;

            signal_power += compare_cpx_power(reference_out.a);
            signal_power += compare_cpx_power(reference_out.b);
